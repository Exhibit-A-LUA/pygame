...
from attack import AttckAnimationSprite
...
class Game:
  ...
  def get_input(self, state, data = None):
    if state == 'attack':
      ...
      AttckAnimationSprite(self.monster, self.attack_frames['green'], self.all_sprites)
  ...
  def apply_attack(self, target, attack):
    ...
    AttckAnimationSprite(target, self.attack_frames[attack_data['animation']], self.all_sprites)
    self.audio[attack_data['animation']].set_volume(0.1)
