...
class Character(Entity):
  def __init__(self, pos, frames, groups, facing_direction, character_data, player, create_dialog, collision_sprites, radius, nurse, notice_sound):
    ...
    self.collosion_rects =  [sprite.rect for sprite in collision_sprites if sprite is not self]
    self.nurse = nurse
  ...
  def has_los(self): # line of sight
    if vector(self.rect.center).distance_to(self.player.rect.center) < self.radius:
      collisions = [bool(rect.clipline(self.rect.center, self.player.rect.center)) for rect in self.collosion_rects]
