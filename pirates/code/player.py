...
class Player(pygame.sprite.Sprite):
  ...
  def platform_move(self, dt):
    if self.platform:
      # if below is hitbox_rect, player falls through sinking platform
      # self.hitbox_rect.topleft += self.platform.direction * self.platform.speed * dt

      # but this version has player bouncing when platform is sinking
  ...
  def get_state(self):
    if self.on_surface['floor']:
      if self.attacking:
        ...
        self.state = 'idle' if self.direction == 0 else 'run'
    else:
      if self.attacking:
